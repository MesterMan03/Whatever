...
pub fn dispatch(
    ...
    match msg {
        ...
        ScriptMessage::SpawnSprite { entity_id, texture, position, scale } => {
            tracing::debug!(mod_id, "SpawnSprite {entity_id} tex={texture} pos={position:?} scale={scale:?}");
        }
        ScriptMessage::MoveEntity { entity_id, position } => {
